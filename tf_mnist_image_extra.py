# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 14:42:37 2018
...
pixels_per_image = shape[1] # 图片对应的像素点数目
...
if (images_count == labels_count) and (shape.size == 1):
        ...
        for i in range(0,images_to_extract):
            for n in range(pixels_per_image):
                if mnist.train.images[i][n] != 0:
                    mnist.train.images[i][n] = 255 # 将像素值不为0的直接转换成255(白色)
    ...
    for i in range(0,images_to_extract):
        img = Image.new("L",(cols,rows)) # 创建图片对象
        for m in range(rows):
            for n in range(cols):
                img.putpixel((n,m), int(mnist.train.images[i][n+m*cols])) # 将图像数据赋给图片对象
